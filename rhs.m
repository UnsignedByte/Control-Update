function [zdot,u]=rhs(currentState,p,K, delta_offset, phi_offset,linear)
...
% end
%(-v^2*delta-b*v*u+g*l*phi)/(h*l) + sin(currentState(1)*5);
%g*sin(phi)/h -(v^2)*tan(delta)/(h*l)-k2*phi_dot*b*v/(h*l);
%(1/h)*...
%    (g*sin(phi) - tan(delta).*...
%       (v.^2/l + b*v_dot/l + tan(phi).*...
%            ((b/l)*v.*phi_dot - (h/l^2)*v.^2.*tan(delta)))...

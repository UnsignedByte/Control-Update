...
% b = zeros(100,1);
% c = ones(100,1)*0.3;
%  p.g = 11.00; %acceleration due to gravity (set to 9.81, of course)
%    p.l = 0.4; %length of wheel base (set to 1.02) 
%    p.b = 0.17; %distance from rear wheel to COM projected onto ground (set to 0.33) 
%    p.h = 0.9; %height of COM in point mass model (set to 0.516)
% delta_offset = [a; b; c];
% %[success,states, stable] = runBicycleTestR(0,0,3,0,0,0,0,0,[11.1,2.4,-6.6], 0, 0,0, 300,  0);
% [success,states, stable] = runBicycleTestR(0,0,3,0,0, 0,0,p,  [11.1 2.4 -6.6], delta_offset, 0,0, 300,  0);
% plotController(0,0,3,0,0, ...
%                 0,0,p,  [11.1 2.4 -6.6], delta_offset, 0,0,  300, 0)
...
[sucess,state,~] = runBicycleTestR(x0,y0,v0,delta0,phi0,phi_dot0,psi0,p, ...
    K,delta_offset,numTimeSteps,0,0, 0);

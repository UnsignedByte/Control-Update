...
x1 =linspace(.2,0.00,50);
y1 = linspace(.2, 0.00,50);
result = zeros(length(x1)*length(y1),4);
trial = 1;
for x=x1 % lag1
    for y =y1 % lag2
        [success, ~, stability, commands] = runBicycleTestR(0,0,v0,0,pi/6,0,0,p,K, 0,x,y,500,0,0);
        % see speedTest.m for argument documentation
...
%Tsum  = successes2(1,4);
% finds the maximum delay that still succeeds.
[~, idx] = max(successes, [], 4);
M = successes(idx, 4);
if(isempty(M))
    T1 = -1;
